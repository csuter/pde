...
output_dir = '/var/www/html/handouts/12_steps_to_navier_stokers/step_5'
doc = hd.handout.Handout(output_dir, title='Step 5')
doc.show()
...
for t in range(nt - 1):
  u[1:, 1:, t+1] = (
      u[1:, 1:, t] -
      c * dt / dx * (u[1:, 1:, t] - u[:-1, 1:, t]) -
      c * dt / dy * (u[1:, 1:, t] - u[1:, :-1, t]))
